...
document.addEventListener('DOMContentLoaded', () => {
  fetch('http://localhost:8000/get-itemsEventi')
    ...
    .then(data => {
        ...
        totalItemsEventiElement.textContent= `Numero totale di eventi catturati: ${data.totalItemsEventi}`;
        } else {
          console.error('elemento non trovato.');
        }
      })
...
//TABELLA 10 SESSIONI 
/*document.addEventListener('DOMContentLoaded', () => {
  fetch('http://localhost:8000/10-sessioni')
    .then(response => response.json())
    .then(data => {
        TabellaUltimeSessioni(data);
    })
    .catch(error => {
      console.error('Errore durante la richiesta al server:', error);
    });

    function TabellaUltimeSessioni (sessioni) {
      const tableBody = document.getElementById('sessioni-table');
